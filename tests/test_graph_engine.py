...
Tests entity extraction, relationship mapping, graph queries, and Neo4j integration.
"""
...
class TestGraphEngine:
    ...
    async def graph_engine(self):
        ...
        with patch('src.core.graph.engine.Neo4jEngine') as mock_neo4j:
            engine = GraphEngine()
            engine.neo4j_client = AsyncMock()
    ...
    async def test_store_entities_in_graph(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.return_value = {"nodes_created": 2, "relationships_created": 1}
        ...
        graph_engine.neo4j_client.execute_query.assert_called()
    ...
    async def test_query_by_entity(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.return_value = mock_query_result
    ...
    async def test_find_connections_between_entities(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.return_value = mock_paths
    ...
    async def test_temporal_query_support(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.return_value = mock_temporal_results
    ...
    async def test_graph_traversal_with_filters(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.return_value = mock_traversal
    ...
    async def test_graph_statistics(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.return_value = mock_stats
    ...
    async def test_memory_integration(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.return_value = {"memory_links_created": 3}
    ...
    async def test_graph_validation(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.return_value = mock_validation
    ...
    async def test_graph_backup_and_restore(self, graph_engine):
        ...
        graph_engine.neo4j_client.create_backup.return_value = backup_result
        ...
        graph_engine.neo4j_client.restore_backup.return_value = restore_result
    ...
    async def test_error_handling_connection_failure(self, graph_engine):
        ...
        graph_engine.neo4j_client.execute_query.side_effect = Exception("Connection to Neo4j failed")
        
        # Execute and verify graceful error handling
        with pytest.raises(Exception, match="Connection to Neo4j failed"):
    ...
    async def test_health_check(self, graph_engine):
        ...
        # Mock health check
        graph_engine.neo4j_client.ping.return_value = True
        ...
        assert health["components"]["neo4j"] == "healthy"
