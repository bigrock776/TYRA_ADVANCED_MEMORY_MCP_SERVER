"""
Integration tests for graph system including Neo4j and Graphiti.
...
class TestGraphIntegration:
    ...
    @pytest.fixture
    def mock_neo4j_engine(self):
        """Create mock Neo4j engine for testing."""
        engine = AsyncMock(spec=Neo4jEngine)
    ...
    @pytest.mark.asyncio
    async def test_neo4j_client_initialization(self, mock_neo4j_engine):
        ...
        with patch('src.core.utils.registry.get_provider', return_value=mock_neo4j_engine):
            await client.initialize(config)
            
            assert client.engine is mock_neo4j_engine
            assert client.config == config
    
    @pytest.mark.asyncio
    async def test_entity_operations_through_client(self, mock_neo4j_engine):
        """Test entity operations through the Neo4j client."""
        client = Neo4jClient()
        client.engine = mock_neo4j_engine
        ...
        mock_neo4j_engine.get_entity.assert_called_with("entity_123")
        mock_neo4j_engine.find_entities.assert_called_with(
            entity_type="programming_language",
    ...
    @pytest.mark.asyncio
    async def test_relationship_operations_through_client(self, mock_neo4j_engine):
        """Test relationship operations through the Neo4j client."""
        client = Neo4jClient()
        client.engine = mock_neo4j_engine
        ...
        # Verify engine was called correctly
        mock_neo4j_engine.get_entity_relationships.assert_called_with(
            entity_id="entity_1",
            relationship_type="created_by",
            direction="both"
        )
    ...
    @pytest.mark.asyncio
    async def test_graph_traversal_operations(self, mock_neo4j_engine):
        ...
        # Verify engine calls
        mock_neo4j_engine.get_connected_entities.assert_called()
        mock_neo4j_engine.find_path.assert_called_with(
            source_entity_id="entity_1",
            target_entity_id="entity_2",
            max_depth=3
        )
    ...
    @pytest.mark.asyncio
    async def test_temporal_graph_queries(self, mock_neo4j_engine):
        ...
        mock_neo4j_engine.find_entities.assert_called()
    
    @pytest.mark.asyncio
    async def test_graph_statistics(self, mock_neo4j_engine):
        """Test graph statistics retrieval."""
        client = Neo4jClient()
        client.engine = mock_neo4j_engine
        
        # Mock stats response
        mock_neo4j_engine.get_stats.return_value = {
            "graph_statistics": {
                "total_entities": 150,
                "total_relationships": 300,
        ...
        # Verify engine was called
        mock_neo4j_engine.get_stats.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_graph_health_monitoring(self, mock_neo4j_engine, mock_graphiti_manager):
        """Test graph system health monitoring."""
        client = Neo4jClient()
        client.engine = mock_neo4j_engine
        
        # Test Neo4j health
        neo4j_health = await mock_neo4j_engine.health_check()
        
        assert neo4j_health["status"] == "healthy"
        assert neo4j_health["response_time"] == 0.023
        assert neo4j_health["graph_stats"]["total_entities"] == 150
        ...
        # Verify calls
        mock_neo4j_engine.health_check.assert_called_once()
        mock_graphiti_manager.health_check.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_integrated_graph_workflow(self, mock_neo4j_engine, mock_graphiti_manager):
        """Test integrated workflow using both Neo4j and Graphiti."""
        client = Neo4jClient()
        client.engine = mock_neo4j_engine
        ...
        mock_neo4j_engine.get_entity.assert_called_once()
        mock_neo4j_engine.get_entity_relationships.assert_called_once()
        mock_graphiti_manager.validate_facts.assert_called_once()
        mock_graphiti_manager.get_entity_timeline.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_graph_error_handling(self, mock_neo4j_engine, mock_graphiti_manager):
        """Test error handling in graph operations."""
        client = Neo4jClient()
        client.engine = mock_neo4j_engine
        
        # Test Neo4j error handling
        mock_neo4j_engine.get_entity.side_effect = Exception("Neo4j connection failed")
        ...
        mock_neo4j_engine.get_entity.side_effect = None
        mock_graphiti_manager.search.side_effect = None
        
        # Test health check with errors
        mock_neo4j_engine.health_check.return_value = {
            "status": "unhealthy",
            "error": "Connection timeout"
        }
        
        health = await mock_neo4j_engine.health_check()
        assert health["status"] == "unhealthy"
        assert "error" in health
    
    @pytest.mark.asyncio
    async def test_graph_performance_monitoring(self, mock_neo4j_engine, mock_graphiti_manager):
        ...
        stats = await mock_neo4j_engine.get_stats()
