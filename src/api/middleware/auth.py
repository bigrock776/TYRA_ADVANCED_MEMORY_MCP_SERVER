...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def _validate_basic_auth(self, request: Request) -> dict:
        ...
        try:
            ...
            # Validate credentials using bcrypt or environment variables
    ...
    def _is_valid_token(self, token: str) -> bool:
        ...
        try:
            ...
            jwt_secret = os.getenv("JWT_SECRET", "tyra-dev-secret-change-in-production")
            ...
            if not payload.get("user_id") or not payload.get("iat"):
                return False
            ...
            # Fallback if PyJWT not available - simple token validation
            logger.warning("PyJWT not available, using simple token validation")
            return len(token) >= 32 and token.startswith("tyra_")
        except Exception as e:
    ...
    def _extract_permissions_from_token(self, token: str) -> list:
        ...
        try:
            ...
            jwt_secret = os.getenv("JWT_SECRET", "tyra-dev-secret-change-in-production")
            payload = jwt.decode(token, jwt_secret, algorithms=["HS256"])
            return payload.get("permissions", ["read"])
        ...
        try:
            ...
            if username == admin_user:
                ...
                if admin_password.startswith("$2b$"):
                    return bcrypt.checkpw(password.encode(), admin_password.encode())
                else:
                    # Plain text comparison (development only)
                    logger.warning("Using plain text password comparison - not secure for production")
