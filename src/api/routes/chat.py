...
async def create_chat_completion(
    ...
    try:
        ...
        # Calculate token usage with proper tokenization
        usage = _calculate_token_usage(request.messages, response_content)
...
async def _generate_response(
    ...
    """Generate a response using local LLM integration."""
    try:
        # Import HTTP client for LLM requests
        import aiohttp
        ...
        system_prompt = "You are Tyra, an intelligent memory-enhanced AI assistant. Use the provided context to give accurate, helpful responses."
        ...
        elif model == ChatModel.BALANCED:
            model_name = "meta-llama/Llama-3.1-40B-Instruct"
...
def _generate_fallback_response(user_message: str, context: str) -> str:
    ...
    response = f"I understand you're asking about: {user_message[:100]}"
    
    if context:
        response += f"\n\nBased on the available context from the memory system, I can see relevant information that relates to your query."
    else:
        response += "\n\nI don't have specific context for this query, but I'm processing your request."
    
    response += "\n\n(Note: LLM service is currently unavailable, this is a fallback response)"
    return response
...
def _calculate_token_usage(messages: List[Any], response_content: str) -> Dict[str, int]:
    """Calculate token usage for messages and response."""
    try:
        # Try to use a proper tokenizer if available
        try:
            import tiktoken
            
            # Use GPT-3.5/4 tokenizer as a reasonable default
            ...
            completion_tokens = _estimate_tokens_from_text(response_content)
        ...
        # Fallback to simple word counting if all else fails
        prompt_words = sum(len(msg.content.split()) for msg in messages)
        completion_words = len(response_content.split())
        ...
        return {
            ...
            "total_tokens": int((prompt_words + completion_words) / 0.75)
        }
...
def _estimate_tokens_from_text(text: str) -> int:
    ...
    # More sophisticated estimation than simple word count
    # Consider punctuation, special characters, and subwords
    
    # Basic character count divided by average chars per token
    char_count = len(text)
    
    # Average characters per token for English text is approximately 4
    # But adjust for different types of content
    if any(char in text for char in "()[]{}"):
        # Code-like content tends to have more tokens per character
        chars_per_token = 3.5
    elif len(text.split()) / len(text) > 0.2:  # Lots of spaces = more words
        # Regular text
        chars_per_token = 4.0
    else:
        # Dense text or special characters
        chars_per_token = 3.8
