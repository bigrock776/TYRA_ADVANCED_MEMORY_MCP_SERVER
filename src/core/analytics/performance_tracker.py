...
class PerformanceTracker:
    ...
    async def _detect_outlier_patterns(self):
        ...
        # Detect clusters of outliers that might indicate systematic issues
        for operation, events in self.events_by_operation.items():
            if len(events) < 20:
                continue
            ...
            # Group by metric type and detect outlier clusters
            metric_outliers = defaultdict(list)
            
            for event in recent_events:
                baseline_key = (event.metric_type, operation)
                if baseline_key in self.baselines:
                    baseline = self.baselines[baseline_key]
                    if baseline.is_anomaly(event.value, self.anomaly_threshold):
                        metric_outliers[event.metric_type].append(event)
            
            # Check for outlier clusters (3+ outliers in a short time window)
            for metric_type, outlier_events in metric_outliers.items():
                if len(outlier_events) >= 3:
                    # Check if outliers are clustered in time
                    times = [e.timestamp for e in outlier_events]
                    time_span = max(times) - min(times)
                    
                    if time_span <= timedelta(minutes=10):  # Cluster within 10 minutes
                        await self._record_outlier_cluster_pattern(
                            operation, metric_type, outlier_events
                        )
    ...
    async def _process_pending_alerts(self):
        ...
        # Process recent alerts and send notifications if needed
        current_time = datetime.utcnow()
        recent_alerts = [
            alert for alert in self.alert_history
            if datetime.fromisoformat(alert['timestamp']) >= current_time - timedelta(minutes=5)
        ]
        ...
        # Log alert summary
        if critical_alerts:
            logger.critical(f"Found {len(critical_alerts)} critical performance alerts in last 5 minutes")
            
        if warning_alerts:
            logger.warning(f"Found {len(warning_alerts)} warning performance alerts in last 5 minutes")
            
        # Send alerts to monitoring systems (if configured)
        await self._send_alerts_to_monitoring(critical_alerts, warning_alerts)
    ...
    async def _record_outlier_cluster_pattern(
        self, operation: str, metric_type: MetricType, outlier_events: List[PerformanceEvent]
    ):
        """Record a detected outlier cluster pattern."""
        pattern_id = f"outlier_cluster_{operation}_{metric_type.value}"
        
        description = (
            f"Outlier cluster detected in {metric_type.value} for {operation} "
            f"({len(outlier_events)} outliers in short time window)"
        )
        ...
        if pattern_id in self.patterns:
            ...
            recommendations = [
                f"Investigate systematic issues causing {metric_type.value} outliers",
                "Check for deployment issues or configuration changes",
            ...
            pattern = PerformancePattern(
                ...
                metric_types=[metric_type],
                confidence=0.8,
                impact_score=len(outlier_events) / 10.0,  # Scale based on cluster size
                first_detected=datetime.utcnow(),
                last_seen=datetime.utcnow(),
                occurrence_count=1,
                related_operations={operation},
            ...
            self.patterns[pattern_id] = pattern
            logger.warning(f"Outlier cluster pattern detected: {description}")

    async def _send_alerts_to_monitoring(
        self, critical_alerts: List[Dict], warning_alerts: List[Dict]
    ):
        """Send alerts to external monitoring systems."""
        try:
            # This could integrate with external monitoring systems
            # For now, we'll log structured alerts
            ...
            if critical_alerts:
                ...
                logger.critical(f"Performance alert summary: {json.dumps(alert_summary)}")
            ...
            if warning_alerts:
                ...
                logger.warning(f"Performance alert summary: {json.dumps(alert_summary)}")
                
        except Exception as e:
            logger.error(f"Failed to send alerts to monitoring: {e}")
