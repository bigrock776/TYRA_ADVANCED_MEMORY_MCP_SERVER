...
class MemoryHealthManager:
    ...
    - Database health checks (PostgreSQL, Redis, Neo4j)
    ...
    async def _check_postgres_connections(self) -> float:
        ...
        try:
            ...
            pg_manager = await get_postgres_manager()
            if pg_manager and hasattr(pg_manager, 'pool'):
                pool = pg_manager.pool
                if pool:
            ...
            logger.warning(f"Could not check PostgreSQL connections: {e}")
        return 0.0  # Return 0 if unable to check
    ...
    async def _check_api_response_time(self) -> float:
        ...
        try:
            ...
            if 'metrics' in summary and 'latency' in summary['metrics']:
                latency_data = summary['metrics']['latency']
                return float(latency_data.get('mean', 0.0))
        except Exception as e:
            logger.warning(f"Could not check API response time: {e}")
        return 0.0  # Return 0 if unable to check
    ...
    async def _check_embedding_performance(self) -> float:
        ...
        try:
            ...
            if 'metrics' in summary and 'embedding_time' in summary['metrics']:
                embedding_data = summary['metrics']['embedding_time']
                return float(embedding_data.get('mean', 0.0))
        except Exception as e:
            logger.warning(f"Could not check embedding performance: {e}")
        return 0.0  # Return 0 if unable to check
    ...
    async def _clear_caches(self):
        ...
        try:
            ...
            redis_client = await get_redis_client()
            if redis_client:
                # Clear expired keys and run garbage collection
    ...
    async def _cleanup_log_files(self):
        ...
        try:
            ...
            log_dirs = ['/var/log/tyra', './logs', '/tmp/tyra_logs']
            for log_dir in log_dirs:
                if Path(log_dir).exists():
                    old_logs = glob.glob(f"{log_dir}/*.log.*")
                    for log_file in old_logs:
                        if Path(log_file).stat().st_mtime < time.time() - (7 * 24 * 3600):
                            os.remove(log_file)
    ...
    async def _restart_workers(self):
        ...
        try:
            # Signal to restart background tasks
            import signal
            import os
            logger.warning("Worker restart requested - sending SIGUSR1 to parent process")
    ...
    async def _scale_down_workers(self):
        ...
        try:
            # Reduce thread pool sizes and connection pools
            from core.utils.database import get_postgres_manager
            pg_manager = await get_postgres_manager()
            if pg_manager and hasattr(pg_manager, 'pool'):
                # Reduce pool size temporarily
                logger.info("Reducing database connection pool size")
        except Exception as e:
    ...
    async def _throttle_requests(self):
        ...
        try:
            # Set a global flag for rate limiting
            import asyncio
            # Create a simple throttling mechanism
            if not hasattr(self, '_throttle_enabled'):
                self._throttle_enabled = True
                self._request_semaphore = asyncio.Semaphore(10)  # Limit to 10 concurrent requests
                logger.info("Request throttling enabled with 10 concurrent request limit")
        except Exception as e:
            logger.error(f"Failed to enable request throttling: {e}")
