"""Enhanced graph client integrating Neo4j and Graphiti for temporal knowledge graphs."""
...
class EnhancedGraphClient(GraphEngine):
    """Enhanced graph client combining Neo4j and Graphiti capabilities."""
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        """Initialize both Neo4j and Graphiti clients."""
        try:
            # Initialize Neo4j client
            await self.neo4j_client.initialize(config.get("neo4j", {}))
            ...
            if self.enable_graphiti and self.graphiti_manager:
                ...
                if graphiti_config.get("enabled", True):
                    try:
                        ...
                        logger.info("Continuing with Neo4j-only mode")
    ...
    async def store_memory(
        ...
        # Store in Neo4j
        try:
            neo4j_result = await self.neo4j_client.store_entity(
                entity_id=memory_id,
                entity_type="Memory",
                properties={
                    "content": content,
    ...
    async def query(
        ...
        Args:
            query: Query string (Cypher for Neo4j, natural language for Graphiti)
        ...
        Returns:
            Query results
        """
        # If query looks like Cypher, use Neo4j
        if query.strip().upper().startswith(('MATCH', 'CREATE', 'DELETE', 'MERGE', 'RETURN')):
            return await self.neo4j_client.execute_query(query, parameters or {})
        ...
        if use_graphiti and self.graphiti_manager:
            try:
                ...
                logger.warning(f"Graphiti query failed, falling back to Neo4j: {e}")
                
        # Fallback to Neo4j with text search
        return await self._text_search_neo4j(query)
        
    async def _text_search_neo4j(self, query: str) -> List[Dict[str, Any]]:
        """Search Neo4j using text-based queries."""
    ...
    async def get_temporal_facts(
        ...
        # Fallback to Neo4j temporal query
        cypher_query = """
    ...
    async def extract_and_store_entities(
        ...
        # Also store basic entities in Neo4j
        try:
            # Basic entity storage in Neo4j
            await self.neo4j_client.store_entity(
                entity_id=source_id,
            ...
            results["neo4j_stored"] = True
            
        except Exception as e:
            logger.error(f"Neo4j entity storage failed: {e}")
            results["neo4j_error"] = str(e)
    ...
    async def validate_knowledge(
        ...
        # Fallback to basic Neo4j validation
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {"neo4j": {}, "graphiti": {}}
        
        # Get Neo4j statistics
        try:
            stats["neo4j"] = await self.neo4j_client.get_statistics()
        except Exception as e:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health = {"overall": "healthy", "neo4j": {}, "graphiti": {}}
