"""
Graphiti Integration for Temporal Knowledge Graphs with Neo4j.
...
try:
    ...
    from graphiti_neo4j import Neo4jGraphitiBridge
...
class GraphitiManager:
    ...
    framework with local LLM integration and Neo4j backend.
    """
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Initialize Neo4j bridge for Graphiti
            neo4j_bridge = Neo4jGraphitiBridge(
                uri=config.get("neo4j_uri", "neo4j://localhost:7687"),
                username=config.get("neo4j_username", "neo4j"),
                password=config.get("neo4j_password", "neo4j"),
                database=config.get("neo4j_database", "neo4j"),
                encrypted=config.get("neo4j_encrypted", False)
            )
            
            # Initialize Graphiti client with Neo4j bridge
            self.graphiti_client = Graphiti(
                llm_client=llm_client,
                embedder=local_embedder,
                reranker=reranker,
                graph_client=neo4j_bridge
            )
    ...
    async def add_memory_to_graph(self, memory_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            # Fetch the memory from the memory store
            ...
            text_content = memory.content if hasattr(memory, 'content') else str(memory)
            
            # Use Graphiti to add memory to graph
            ...
            # Count entities and relationships added
            entities_added = len(episode_result.get("entities", []))
            relationships_added = len(episode_result.get("relationships", []))
            
            logger.info(
                f"Added memory {memory_id} to graph",
            ...
            return {
                ...
                "entities_added": entities_added,
                "relationships_added": relationships_added,
                "message": f"Successfully added memory to graph with {entities_added} entities and {relationships_added} relationships",
            }
