...
async def rollback_configuration() -> bool:
    ...
    try:
        ...
        if config_path.exists():
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            current_backup = backup_dir / f"pre_rollback_{timestamp}.json"
            
            with open(config_path, 'r') as f:
                current_config = f.read()
            
            with open(current_backup, 'w') as f:
                json.dump({"config": current_config, "timestamp": timestamp}, f, indent=2)
                
        # Write backup data to config
        with open(config_path, 'w') as f:
            f.write(backup_data.get("config", ""))
            
        print(f"✅ Configuration rolled back from {latest_backup.name}")
        return True
...
def main():
    ...
    try:
        if args.status:
            ...
            success = await rollback_configuration()
            print("✅ Configuration rollback completed" if success else "❌ Configuration rollback failed")
            sys.exit(0 if success else 1)
