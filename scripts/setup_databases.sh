...
# This script sets up PostgreSQL, Neo4j, and Redis for the Tyra Memory System
...
check_requirements() {
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        if command -v apt-get &> /dev/null; then
            ...
            error "Unsupported Linux distribution. Please install PostgreSQL, Neo4j, and Redis manually."
            exit 1
        fi
...
EOF
    ...
    log "Neo4j installation completed."
}
...
initialize_databases() {
    ...
    if command -v cypher-shell &> /dev/null; then
        if echo "RETURN 1;" | cypher-shell -a bolt://127.0.0.1:7687 -u neo4j -p neo4j &>/dev/null; then
            log "Neo4j connection successful"
        else
            warning "Neo4j connection failed - check if service is running"
        fi
    else
        info "cypher-shell not available - install neo4j-client for testing"
    fi
...
# Backup Neo4j (if available)
if command -v cypher-shell &> /dev/null; then
    echo "Backing up Neo4j..."
    echo "CALL apoc.export.cypher.all('/tmp/neo4j_backup.cypher', {format: 'cypher-shell'});" | cypher-shell -a bolt://$NEO4J_HOST:$NEO4J_PORT -u $NEO4J_USER -p $NEO4J_PASSWORD
    cp /tmp/neo4j_backup.cypher "$BACKUP_DIR/neo4j_$TIMESTAMP.cypher"
fi
...
main() {
    ...
    log "- Neo4j: Graph database ready for knowledge graph operations"
