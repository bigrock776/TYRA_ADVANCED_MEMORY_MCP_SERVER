...
# Test Neo4j
test_neo4j() {
    log "Testing Neo4j connection and functionality..."
    ...
    if command -v cypher-shell &> /dev/null; then
        if echo "RETURN 1;" | cypher-shell -a bolt://$NEO4J_HOST:$NEO4J_PORT -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" &>/dev/null; then
            info "✅ Neo4j connection successful (cypher-shell)"
...
try:
    ...
    sys.exit(1)
" &>/dev/null; then
            info "✅ Neo4j connection successful (Python driver)"
            connection_successful=true
        fi
    fi
    ...
    if [[ "$connection_successful" == false ]]; then
        ...
        error "   Host: $NEO4J_HOST:$NEO4J_PORT"
        error "   Tried: cypher-shell, Python neo4j driver"
        return 1
    fi
    
    # Test schema existence (using first available connection method)
    info "Testing Neo4j schema..."
    local test_query="MATCH (s:System {id: 'tyra_memory_system'}) RETURN s.name;"
    local result=""
    
    if command -v cypher-shell &> /dev/null; then
        result=$(echo "$test_query" | cypher-shell -a bolt://$NEO4J_HOST:$NEO4J_PORT -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" 2>/dev/null || echo "")
    fi
    
    if [[ "$result" == *"Tyra Advanced Memory System"* ]]; then
        info "✅ Neo4j schema initialized"
    else
        warning "⚠️ Neo4j schema may not be initialized"
    fi
    ...
    if command -v cypher-shell &> /dev/null; then
        if echo "$graph_test_query" | cypher-shell -a bolt://$NEO4J_HOST:$NEO4J_PORT -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" &>/dev/null; then
    ...
    local start_time=$(date +%s.%N)
    echo "MATCH (n) RETURN count(n);" | cypher-shell -a bolt://$NEO4J_HOST:$NEO4J_PORT -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" &>/dev/null || true
    local end_time=$(date +%s.%N)
    local duration=$(echo "$end_time - $start_time" | bc -l)
    local duration_ms=$(echo "$duration * 1000" | bc -l | cut -d. -f1)
    
    if (( duration_ms < 50 )); then
        info "✅ Neo4j query performance: ${duration_ms}ms (excellent)"
    elif (( duration_ms < 200 )); then
        info "⚠️ Neo4j query performance: ${duration_ms}ms (acceptable)"
    else
        warning "⚠️ Neo4j query performance: ${duration_ms}ms (slow)"
    fi
    
    NEO4J_STATUS="✅"
    log "Neo4j tests completed successfully"
    return 0
}
...
Neo4j:
- Graph schema: $([ "$NEO4J_STATUS" = "✅" ] && echo "✅ Initialized" || echo "❌ Not tested")
- Graph operations: $([ "$NEO4J_STATUS" = "✅" ] && echo "✅ Working" || echo "❌ Not tested")
...
Overall Status: $([ "$POSTGRES_STATUS$NEO4J_STATUS$REDIS_STATUS" = "✅✅✅" ] && echo "✅ READY" || echo "❌ NEEDS ATTENTION")

$([ "$POSTGRES_STATUS$NEO4J_STATUS$REDIS_STATUS" = "✅✅✅" ] && echo "All databases are properly configured and ready for the Tyra Memory System." || echo "Some databases need attention before the system can be fully operational.")
...
$([ "$NEO4J_STATUS" != "✅" ] && echo "- Fix Neo4j connection and initialization")  
$([ "$REDIS_STATUS" != "✅" ] && echo "- Fix Redis connection and initialization")
$([ "$POSTGRES_STATUS$NEO4J_STATUS$REDIS_STATUS" = "✅✅✅" ] && echo "- System is ready for production use")
...
EOF
    ...
    if [[ "$POSTGRES_STATUS$NEO4J_STATUS$REDIS_STATUS" = "✅✅✅" ]]; then
