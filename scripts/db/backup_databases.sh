...
# Creates backups of PostgreSQL, Neo4j, and Redis data
...
# 2. Backup Neo4j
echo -e "\n${BLUE}🔗 Backing up Neo4j...${NC}"

if check_service "Neo4j" "$NEO4J_PORT" "$NEO4J_HOST"; then
    NEO4J_BACKUP_FILE="$BACKUP_DIR/neo4j_${TIMESTAMP}.cypher"

    echo -e "${YELLOW}📁 Creating Neo4j backup: $NEO4J_BACKUP_FILE${NC}"
...
EOF
        ...
        if cypher-shell -u neo4j -p neo4j123 --address "bolt://$NEO4J_HOST:$NEO4J_PORT" < "/tmp/neo4j_export_${TIMESTAMP}.cypher" > "$NEO4J_BACKUP_FILE" 2>/dev/null; then
            rm "/tmp/neo4j_export_${TIMESTAMP}.cypher"
            gzip "$NEO4J_BACKUP_FILE"
            echo -e "${GREEN}✓ Neo4j backup completed: ${NEO4J_BACKUP_FILE}.gz${NC}"

            BACKUP_SIZE=$(du -h "${NEO4J_BACKUP_FILE}.gz" | cut -f1)
            echo -e "${BLUE}📏 Backup size: $BACKUP_SIZE${NC}"
        else
            echo -e "${RED}✗ Neo4j backup failed${NC}"
            rm -f "/tmp/neo4j_export_${TIMESTAMP}.cypher"
        fi
    else
        echo -e "${YELLOW}⚠️ cypher-shell not found, creating basic backup placeholder${NC}"
        echo "# Neo4j backup placeholder - install cypher-shell for full backup" > "$NEO4J_BACKUP_FILE"
        echo "# Backup created at: $(date)" >> "$NEO4J_BACKUP_FILE"
        gzip "$NEO4J_BACKUP_FILE"
        echo -e "${YELLOW}⚠️ Basic Neo4j backup created: ${NEO4J_BACKUP_FILE}.gz${NC}"
    fi
else
    echo -e "${YELLOW}⚠️ Neo4j is not running, skipping backup${NC}"
fi
