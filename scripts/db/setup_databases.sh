...
# Sets up PostgreSQL with pgvector, Neo4j, and Redis
...
if check_service "Neo4j" "$NEO4J_PORT" "$NEO4J_HOST"; then
    ...
    if command_exists cypher-shell; then
        if echo "RETURN 1;" | cypher-shell -u neo4j -p neo4j123 --address "bolt://$NEO4J_HOST:$NEO4J_PORT" >/dev/null 2>&1; then
            echo -e "${GREEN}✓ Neo4j connection successful${NC}"
        else
            echo -e "${YELLOW}⚠️ Neo4j connection failed${NC}"
        fi
    else
        echo -e "${YELLOW}⚠️ cypher-shell not found, skipping connection test${NC}"
        echo "Install cypher-shell for testing: https://neo4j.com/docs/operations-manual/current/tools/cypher-shell/"
    fi
else
    echo -e "${YELLOW}⚠️ Neo4j is not running.${NC}"
    echo "Please install and start Neo4j:"
    echo "Docker: docker run -p 7687:7687 -p 7474:7474 neo4j:5.15-community"
    echo "Or follow: https://neo4j.com/docs/getting-started/"
fi
