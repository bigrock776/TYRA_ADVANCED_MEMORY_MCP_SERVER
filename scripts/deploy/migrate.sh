...
check_neo4j_connection() {
    ...
    # Extract Neo4j host and port from URL
    NEO4J_HOST=$(echo "$NEO4J_URL" | sed -n 's|bolt://\([^:]*\).*|\1|p')
    NEO4J_PORT=$(echo "$NEO4J_URL" | sed -n 's|bolt://[^:]*:\([0-9]*\).*|\1|p')
    NEO4J_PORT=${NEO4J_PORT:-7687}

    if echo "RETURN 1;" | cypher-shell -u neo4j -p neo4j -a "bolt://$NEO4J_HOST:$NEO4J_PORT" > /dev/null 2>&1; then
        success "Neo4j connection successful"
        return 0
    else
        warning "Neo4j connection failed or cypher-shell not available"
        return 1
    fi
}
...
apply_python_migration() {
    ...
    export DATABASE_URL REDIS_URL NEO4J_URL
...
migrate() {
    ...
    check_neo4j_connection || warning "Neo4j connection failed - graph features may not work"
